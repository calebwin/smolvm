{
  ...
  releases = {
    x86_64-linux = {
      ...
      hash = "sha256-54MriQtVH5tobIfEAJspZUCZBYZ4kmRdWWMcrPdpX8Y=";
    };
    aarch64-linux = {
      asset = "smolvm-${version}-linux-arm64.tar.gz";
      root = "smolvm-${version}-linux-arm64";
      hash = "sha256-oKfRsnzwmRdd3PecmxxVt4HtKuY13QoDw8rnbkuMKYU=";
    };
    aarch64-darwin = {
      asset = "smolvm-${version}-darwin-arm64.tar.gz";
      root = "smolvm-${version}-darwin-arm64";
      hash = "sha256-xp0MkKQ4mPwDBvZmmfiFehIlEDdcEjBKYhqEyuObAtY=";
    };
  };
