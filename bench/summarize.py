#!/usr/bin/env python3
"""Summarize bench.sh results without mixing workloads or run lengths.
...
def main():
    ...
    for path in sorted(glob.glob(os.path.join(d, "*.json"))):
        ...
        # `results/` also contains durable multi-run evidence summaries. Only
        # raw bench.sh records have this complete identity tuple.
        if all(
            key in result
            for key in ("arm", "n", "steps", "cpus_per_learner", "learners")
        ):
            runs.append(result)
    if not runs:
    ...
    for r in runs:
        method = next(
            (learner.get("method") for learner in r["learners"] if learner.get("method")),
            r.get("workload", "dpo"),
        )
        source_id = r.get("workload_md5") or r.get("workload", method)
        groups.setdefault(
            (
                method,
                source_id,
    ...
    print(
        f"{'workload':<18} {'arm':<7} {'N':>3} {'steps':>5} {'bxseq':>9} "
        f"{'reps':>4} {'wall_s':>8} {'throughput':>10} {'peak_GPU':>9} "
        f"{'golden_s':>9} {'done':>7}"
    )
    print("-" * 108)
    rows = {}
    for (workload, source_id, arm, n, steps, cpus, batch, maxseq), rs in sorted(
        groups.items()
    ):
        wall = med([r["wall_s"] for r in rs])
        agg = med(
            [
                r.get("tail_agg_tok_s", r.get("exact_agg_tok_s", r["agg_tok_s"]))
                for r in rs
            ]
        )
        ...
        rows[(workload, source_id, arm, n, steps, cpus, batch, maxseq)] = (
            wall,
            agg,
            peak,
        )
        ...
        print(
            f"{workload:<18} {arm:<7} {n:>3} {steps:>5} {shape:>9} "
            f"{len(rs):>4} {fmt(wall,'',1):>8} {fmt(agg,'',1):>10} "
    ...
    print()
    for (workload, source_id, arm, n, steps, cpus, batch, maxseq), (
        wall,
        agg,
        peak,
        ...
        if arm != "fork":
            continue
        base = rows.get(
            (workload, source_id, "native", n, steps, cpus, batch, maxseq)
        )
        if not base:
            continue
        bw, ba, bp = base
        print(
            f"{workload}, N={n}, steps={steps}, {cpus} cpu/learner, "
            f"batch {batch}x{maxseq} — fork vs native:"
        )
        if bw and wall:
            print(f"  wall:      {wall:.1f}s vs {bw:.1f}s   ({wall/bw:.2f}x {'slower' if wall > bw else 'faster'})")
        if ba and agg:
            print(
                f"  throughput: {agg:.1f} vs {ba:.1f}   "
                f"({ba/agg:.2f}x {'slower' if agg < ba else 'faster'})"
            )
        if bp and peak:
